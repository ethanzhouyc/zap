...
const conformanceParser = require('../conformance/conform-xml-parser')
  ...
  if ('command' in cluster) {
    ...
    cluster.command.forEach((command) => {
      ...
      let cmd = {
        ...
        conformance: conformanceParser.parseConformanceFromXML(command),
  ...
  if ('event' in cluster) {
    ...
    cluster.event.forEach((event) => {
      let ev = {
        ...
        name: event.$.name,
        side: event.$.side,
        conformance: conformanceParser.parseConformanceFromXML(event),
  ...
  if ('attribute' in cluster) {
    ...
    cluster.attribute.forEach((attribute) => {
      ...
      let att = {
        ...
        conformance: conformanceParser.parseConformanceFromXML(attribute),
        min: attribute.$.min,
        max: attribute.$.max,
        minLength: 0,
  ...
  if ('features' in cluster) {
    ...
    cluster.features[0].feature.forEach((feature) => {
      let f = {
        ...
        conformance: conformanceParser.parseConformanceFromXML(feature)
      }
        ...
        cluster.include.forEach((include) => {
          ...
          if ('features' in include) {
            include.features[0].feature.forEach((f) => {
              features.push({
                code: f.$.code,
                conformance: conformanceParser.parseConformanceFromXML(f)
              })
            })
          }
          ret.clusters.push({
