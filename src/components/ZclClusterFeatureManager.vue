          ...
          <q-tr :props="props" class="table_body">
            <q-td key="enabled" :props="props" auto-width>
              <q-toggle
                ...
                @update:model-value="
                  (val) => {
                    onToggleDeviceTypeFeature(props.row, val, 'cluster')
                  }
                "
              />
            </q-td>
      ...
      <q-dialog v-model="showDialog" persistent>
        <q-card>
          <q-card-section>
            ...
            <div v-if="featuresToUpdate.length > 0">
              <div
                class="text-body1"
                style="margin-top: 15px; padding-left: 20px"
              >
              ...
              <ul>
                <li
                  v-for="(feature, index) in processElementsForDialog(
                    featuresToUpdate
                  )"
                  :key="'event' + index"
                  style="margin-bottom: 10px"
...
export default {
  ...
  methods: {
    isToggleDisabled(conformance) {
      ...
      return (
        conformance == dbEnum.conformance.disallowed ||
        conformance == dbEnum.conformance.deprecated
      )
    }
    // onToggleClusterFeature(featureData, inclusionList) {
